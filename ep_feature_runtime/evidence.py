...
from .risk_compat import selective_ep_trigger
...
class NBOGuidedCalibrator:
    ...
    def calibrate_hij(
        ...
        # Exact environment signatures are intentionally diagnostic-only.
        # Their training vocabulary is too sparse for hard OOD routing, while
        # the coarser bond-class dictionary provides stable chemical coverage.
        ...
        # Unseen elements remain a hard OOD signal. Unseen chemical bond
        # classes are a soft hint; exact environment signatures no longer
        # participate in the trigger because they over-select local geometry.
        unknown_env_hint = torch.maximum(
            unknown_atom.to(device=device, dtype=raw_delta_ratio.dtype),
            unknown_class.to(device=device, dtype=raw_delta_ratio.dtype) * 0.5,
        )
        ...
        # Candidate selective route.  Production behaviour remains unchanged
        # unless an explicitly validated joint local-error probability is set
        # by the caller.  In the candidate route risk is used exactly once;
        # EP/NBO evidence supplies physical action authority and may open only a
        # bounded special-interaction floor.
        external_p_error = getattr(self, "_external_local_error_probability", None)
        selective_debug: Dict[str, torch.Tensor] = {}
        selective_route = (
            isinstance(external_p_error, torch.Tensor)
            and external_p_error.numel() == physics_need.numel()
        )
        if selective_route:
            ep_evidence = torch.maximum(
                xh_interaction_score.to(device=device, dtype=physics_need.dtype),
                physical_special_score.to(device=device, dtype=physics_need.dtype),
            )
            trigger, selective_debug = selective_ep_trigger(
                external_p_error.to(device=device, dtype=physics_need.dtype),
                ep_evidence,
                physics_need,
                nbo_reliability,
                error_low=float(getattr(self, "hij_selective_error_low", 0.20)),
                error_high=float(getattr(self, "hij_selective_error_high", 0.60)),
                ep_low=float(getattr(self, "hij_selective_ep_low", 0.35)),
                ep_high=float(getattr(self, "hij_selective_ep_high", 0.75)),
                strong_ep_floor=float(getattr(self, "hij_selective_strong_ep_floor", 0.35)),
            )
        else:
            # Canonical production route retained verbatim.
            trigger = torch.clamp(
        ...
        # The selective route must not re-introduce legacy chemical/ENK risk in
        # chemistry and mode floors after p_error has already been consumed.
        routed_env_need = torch.zeros_like(env_need) if selective_route else env_need
        ...
        if selective_route:
            special_score = torch.maximum(
                xh_interaction_score.to(device=device, dtype=trigger.dtype),
                physical_special_score.to(device=device, dtype=trigger.dtype),
            ).clamp(0.0, 1.0)
        else:
        ...
        self.last_hij_debug = {
            ...
            "selective_risk_enabled": float(selective_route),
            "mean_external_p_error": float(external_p_error.mean().detach().cpu().item()) if selective_route else 0.0,
            "mean_selective_risk_gate": float(selective_debug["risk_gate"].mean().detach().cpu().item()) if selective_debug else 0.0,
            "mean_selective_ep_gate": float(selective_debug["ep_gate"].mean().detach().cpu().item()) if selective_debug else 0.0,
            "mean_selective_action_need": float(selective_debug["action_need"].mean().detach().cpu().item()) if selective_debug else 0.0,
            "mean_selective_physical_authority": float(selective_debug["physical_authority"].mean().detach().cpu().item()) if selective_debug else 0.0,
            "hij_policy": policy,
    ...
    def calibrate_dedipole(
        ...
        # ── Step 1: isotropic trace correction (existing) ──────────────
        if self.train_stats is not None:
        ...
        # ── Step 2: bond-directional anisotropic correction (NEW) ──────
