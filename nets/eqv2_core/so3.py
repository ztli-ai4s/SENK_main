...
class SO3_Rotation(torch.nn.Module):
    ...
    def set_wigner(self, rot_mat3x3, detach=True, stable=False):
        self.device, self.dtype = rot_mat3x3.device, rot_mat3x3.dtype
        length = len(rot_mat3x3)
        self.wigner = self.RotationToWignerDMatrix(
            rot_mat3x3,
            0,
            self.lmax,
            detach=detach,
    ...
    def RotationToWignerDMatrix(
        ...
        if stable:
            eps = 1.0e-12
            x = x / torch.sqrt(torch.sum(x * x, dim=-1, keepdim=True)).clamp_min(eps)
        ...
        if stable:
            eps = 1.0e-12
            r00 = R[..., 0, 0]
            r00_safe = torch.where(r00.abs() < eps, torch.full_like(r00, eps), r00)
            gamma = torch.atan2(R[..., 0, 2], r00_safe)
        else:
